...
```bash
oc apply -k 05-ocp-accelerator-operators/bare-metal-a100-ib/01-operator-subscriptions/
```
...
```bash
oc apply -k 05-ocp-accelerator-operators/bare-metal-a100-ib/02-nfd-operands/
```
...
```bash
oc apply -k 05-ocp-accelerator-operators/bare-metal-a100-ib/03-worker-gpu-rdma-config/
```
...
```bash
oc apply -k 05-ocp-accelerator-operators/bare-metal-a100-ib/15-nvidia-network-operator/
```
...
```bash
oc apply -k 05-ocp-accelerator-operators/bare-metal-a100-ib/20-gpu-readiness/
```
...
```bash
oc apply -k 05-ocp-accelerator-operators/bare-metal-a100-ib/21-gpu-operands/
```
...
After all steps complete, proceed to [07-rdma-validation/](../../07-rdma-validation/) for RDMA connectivity and bandwidth tests.
